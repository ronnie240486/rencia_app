        ...
        {groupHealth.length > 0 && (
          <Card>
            ...
            <CardContent className="grid gap-2 sm:grid-cols-2">
              {groupHealth.map((group) => <div key={group.group} className="rounded-lg border p-3 space-y-2"><div className="flex items-center justify-between gap-3"><div><p className="font-medium text-sm">{group.group}</p><p className="text-xs text-muted-foreground">{group.currentErrors ? `${group.currentErrors} falha(s) atual(is)` : "Todos os Hosts respondendo agora"} · {group.errors} falha(s) no histórico</p></div><Badge className={group.health === "healthy" ? "bg-emerald-600" : group.health === "critical" ? "bg-red-600" : group.health === "attention" ? "bg-amber-600" : "bg-slate-500"}>{group.health === "healthy" ? "Saudável" : group.health === "critical" ? "Crítico" : group.health === "attention" ? "Atenção" : "Sem testes"}</Badge></div>{group.dns?.map((dns) => <div key={dns.id} className="rounded-md bg-muted/40 px-2 py-1.5 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-medium">{dns.titulo}</span><span className={dns.status === "error" ? "text-red-600 font-medium" : dns.status === "success" ? "text-emerald-600" : "text-muted-foreground"}>{dns.status === "error" ? "Falhou" : dns.status === "success" ? "Funcionando" : dns.message?.includes("Host respondeu") ? "Host respondeu" : "Sem teste"}</span></div><p className="font-mono text-[11px] text-muted-foreground truncate">{dns.host}</p>{dns.status === "error" && <p className="text-red-600">Falha atual: {dns.message}{dns.statusCode ? ` (HTTP ${dns.statusCode})` : ""}{dns.checkedAt ? ` · ${new Date(dns.checkedAt).toLocaleString("pt-BR")}` : ""}</p>}{dns.status !== "error" && dns.lastFailure && <p className="text-amber-700">Falhou anteriormente: {dns.lastFailure.message}{dns.lastFailure.statusCode ? ` (HTTP ${dns.lastFailure.statusCode})` : ""} · {new Date(dns.lastFailure.checkedAt).toLocaleString("pt-BR")}</p>}</div>)}</div>)}
            </CardContent>
          </Card>
        )}
