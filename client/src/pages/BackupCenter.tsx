...
import { Download, HardDrive, History, Loader2, Play, RefreshCw, ShieldCheck, ShieldOff, RotateCcw, Trash2 } from "lucide-react";
...
export default function BackupCenter() {
  ...
  const busy = runNow.isPending || enable.isPending || disable.isPending || restore.isPending || remove.isPending || clearHistory.isPending;
  ...
  return <AdminLayout title="Backup Automático"><div className="space-y-6">
    ...
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle className="text-base">Histórico de cópias</CardTitle><Button size="sm" variant="outline" className="gap-1.5 text-destructive" disabled={!snapshots.length || busy} onClick={() => { if (window.confirm("Remover todos os backups do histórico? Depois será necessário criar uma nova cópia para restaurar dados.")) clearHistory.mutate(); }}><Trash2 size={14} /> Limpar</Button></div><CardDescription>Baixe, restaure ou remova uma cópia individual. Remover retira a cópia do histórico do painel.</CardDescription></CardHeader><CardContent className="p-0"><div className="space-y-3 p-4 md:hidden">{snapshots.map((snapshot) => <div key={snapshot.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{date(snapshot.createdAt)}</p><p className="mt-1 text-xs text-muted-foreground">{size(snapshot.fileSize)}</p></div><Badge variant={snapshot.type === "automatic" ? "secondary" : "default"}>{snapshot.type === "automatic" ? "Automático" : "Manual"}</Badge></div><div className="mt-4 grid grid-cols-2 gap-2"><Button asChild size="sm" variant="outline" className="gap-1.5"><a href={snapshot.storageUrl} target="_blank" rel="noreferrer"><Download size={14} /> Baixar</a></Button><Button size="sm" variant="outline" className="gap-1.5" disabled={busy} onClick={() => confirmRestore(snapshot.id)}><RotateCcw size={14} /> Restaurar</Button></div><Button size="sm" variant="ghost" className="mt-2 w-full gap-1.5 text-muted-foreground hover:text-destructive" disabled={busy} onClick={() => { if (window.confirm("Remover este backup do histórico?")) remove.mutate({ snapshotId: snapshot.id }); }}><Trash2 size={14} /> Remover cópia</Button></div>)}{!query.isLoading && !snapshots.length && <p className="py-10 text-center text-sm text-muted-foreground">Nenhum backup criado ainda. Use “Criar agora” para gerar a primeira cópia.</p>}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-sm"><thead className="border-y bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-5 py-3">Data</th><th className="px-5 py-3">Tipo</th><th className="px-5 py-3">Tamanho</th><th className="px-5 py-3">Ações</th></tr></thead><tbody>{snapshots.map((snapshot) => <tr key={snapshot.id} className="border-b last:border-0"><td className="px-5 py-4">{date(snapshot.createdAt)}</td><td className="px-5 py-4"><Badge variant={snapshot.type === "automatic" ? "secondary" : "default"}>{snapshot.type === "automatic" ? "Automático" : "Manual"}</Badge></td><td className="px-5 py-4">{size(snapshot.fileSize)}</td><td className="px-5 py-4"><div className="flex gap-2"><Button asChild size="sm" variant="outline" className="gap-1.5"><a href={snapshot.storageUrl} target="_blank" rel="noreferrer"><Download size={14} /> Baixar</a></Button><Button size="sm" variant="outline" className="gap-1.5" disabled={busy} onClick={() => confirmRestore(snapshot.id)}><RotateCcw size={14} /> Restaurar</Button><Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground hover:text-destructive" disabled={busy} onClick={() => { if (window.confirm("Remover este backup do histórico?")) remove.mutate({ snapshotId: snapshot.id }); }}><Trash2 size={15} /></Button></div></td></tr>)}{!query.isLoading && !snapshots.length && <tr><td colSpan={4} className="px-5 py-14 text-center text-muted-foreground">Nenhum backup criado ainda. Use “Criar agora” para gerar a primeira cópia.</td></tr>}</tbody></table></div></CardContent></Card>
  </div></AdminLayout>;
}
