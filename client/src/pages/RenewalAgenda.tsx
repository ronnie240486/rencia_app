...
export default function RenewalAgenda() {
  ...
  const exportAgenda = () => downloadCsv(`agenda-renovacao-${period}-dias.csv`, ["Cliente", "MAC", "Telefone", "Vencimento", "Dias", "Situação", "Status"], items.map((item) => [item.nomeServer, item.mac, item.telefone, item.dataExpiracao, item.days, label[item.bucket], item.status]));
