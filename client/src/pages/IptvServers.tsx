...
import { AlertTriangle, CalendarClock, CheckCircle2, Clock3, ExternalLink, Loader2, MessageCircle, Pencil, Plus, Power, RefreshCw, Server, Trash2 } from "lucide-react";
...
type Draft = { id?: number; name: string; server: string; expiresAt: string; reminderDays: number; isActive: boolean };
const emptyDraft = (): Draft => ({ name: "", server: "", expiresAt: "", reminderDays: 3, isActive: true });
...
export default function IptvServers() {
  ...
  const openEdit = (server: NonNullable<typeof overview.data>["servers"][number]) => { setDraft({ id: server.id, name: server.name, server: server.server, expiresAt: toDateInput(server.expiresAt), reminderDays: server.reminderDays, isActive: server.isActive }); setDialogOpen(true); };
  const submit = () => {
    if (!draft.name.trim() || !draft.server.trim() || !draft.expiresAt) { toast.error("Preencha nome, servidor e vencimento."); return; }
    if (draft.id) update.mutate({ ...draft, id: draft.id, name: draft.name.trim(), server: draft.server.trim() });
    else create.mutate({ name: draft.name.trim(), server: draft.server.trim(), expiresAt: draft.expiresAt, reminderDays: draft.reminderDays });
  };
  ...
  return <AdminLayout title="Servidores IPTV">
    <div className="mx-auto max-w-6xl space-y-5 p-1 sm:p-3">
      ...
      <Card><CardHeader><CardTitle className="text-base">Servidores cadastrados</CardTitle><CardDescription>Use o botão do WhatsApp quando quiser abrir a mensagem pronta do vencimento.</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[780px] text-sm"><thead className="border-y bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-5 py-3">Nome</th><th className="px-5 py-3">Servidor</th><th className="px-5 py-3">Vencimento</th><th className="px-5 py-3">Aviso</th><th className="px-5 py-3 text-right">Ações</th></tr></thead><tbody>{overview.data?.servers.map((server) => { const alert = overview.data?.expiring.find((item) => item.id === server.id); const days = alert?.daysUntilExpiration; return <tr key={server.id} className="border-b last:border-0"><td className="px-5 py-4"><p className="font-semibold">{server.name}</p><Badge variant={server.isActive ? "secondary" : "outline"} className="mt-1 text-[10px]">{server.isActive ? "Ativo" : "Pausado"}</Badge></td><td className="max-w-[240px] px-5 py-4"><p className="truncate font-mono text-xs" title={server.server}>{server.server}</p></td><td className="px-5 py-4"><p>{new Date(server.expiresAt).toLocaleDateString("pt-BR", { timeZone: "UTC" })}</p>{typeof days === "number" ? <p className="mt-1 text-xs font-medium text-amber-700 dark:text-amber-400">{expirationLabel(days)}</p> : <p className="mt-1 text-xs text-muted-foreground">Avisar {server.reminderDays} dia(s) antes</p>}</td><td className="px-5 py-4"><Badge variant={expiringIds.has(server.id) ? "destructive" : "outline"}>{expiringIds.has(server.id) ? "Atenção" : "Em dia"}</Badge></td><td className="px-5 py-4"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" title="Abrir mensagem no WhatsApp" onClick={() => prepareWhatsApp.mutate({ id: server.id })} disabled={prepareWhatsApp.isPending}><MessageCircle size={16} /></Button><Button size="icon" variant="ghost" title="Editar" onClick={() => openEdit(server)}><Pencil size={16} /></Button><Button size="icon" variant="ghost" className="text-destructive hover:text-destructive" title="Remover" onClick={() => { if (window.confirm(`Remover o servidor ${server.name}?`)) remove.mutate({ id: server.id }); }} disabled={remove.isPending}><Trash2 size={16} /></Button></div></td></tr>; })}{!overview.isLoading && !overview.data?.servers.length && <tr><td colSpan={5} className="px-5 py-14 text-center text-muted-foreground"><Server className="mx-auto mb-3 opacity-40" size={30} /><p className="font-medium">Nenhum servidor cadastrado.</p><p className="mt-1 text-xs">Cadastre o primeiro para acompanhar os vencimentos.</p></td></tr>}</tbody></table></div></CardContent></Card>
    ...
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{draft.id ? "Editar servidor" : "Cadastrar servidor"}</DialogTitle><DialogDescription>Este cadastro é independente e não cria cliente, MAC ou lista.</DialogDescription></DialogHeader><div className="grid gap-4 py-2"><div className="grid gap-2"><Label htmlFor="server-name">Nome</Label><Input id="server-name" value={draft.name} onChange={(event) => setDraft((value) => ({ ...value, name: event.target.value }))} placeholder="Ex.: Servidor Principal" /></div><div className="grid gap-2"><Label htmlFor="server-address">Servidor</Label><Input id="server-address" value={draft.server} onChange={(event) => setDraft((value) => ({ ...value, server: event.target.value }))} placeholder="Ex.: https://servidor.exemplo.com" /></div><div className="grid gap-2 sm:grid-cols-2"><div className="grid gap-2"><Label htmlFor="server-expiration">Vencimento</Label><Input id="server-expiration" type="date" value={draft.expiresAt} onChange={(event) => setDraft((value) => ({ ...value, expiresAt: event.target.value }))} /></div><div className="grid gap-2"><Label htmlFor="server-reminder">Avisar antes</Label><Input id="server-reminder" type="number" min={0} max={30} value={draft.reminderDays} onChange={(event) => setDraft((value) => ({ ...value, reminderDays: Math.min(30, Math.max(0, Number(event.target.value) || 0)) }))} /><p className="text-xs text-muted-foreground">Quantidade de dias.</p></div></div>{draft.id && <div className="flex items-center justify-between rounded-lg border p-3"><div><p className="text-sm font-medium">Servidor ativo</p><p className="text-xs text-muted-foreground">Pausado não gera avisos.</p></div><Switch checked={draft.isActive} onCheckedChange={(checked) => setDraft((value) => ({ ...value, isActive: checked }))} /></div>}</div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Cancelar</Button><Button onClick={submit} disabled={isSaving} className="btn-save gap-2">{isSaving && <Loader2 className="animate-spin" size={15} />} Salvar</Button></DialogFooter></DialogContent></Dialog>
  </AdminLayout>;
}
