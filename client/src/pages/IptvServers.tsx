...
import { AlertTriangle, CalendarClock, CheckCircle2, CircleDollarSign, Clock3, ExternalLink, Loader2, MessageCircle, Mic, MicOff, Pencil, Plus, Power, RefreshCw, Search, Server, Trash2 } from "lucide-react";
...
export default function IptvServers() {
  ...
  const filteredServers = useMemo(() => (overview.data?.servers ?? []).filter((server) => {
    return matchesIptvServerSearch(server, normalizedSearch);
  }), [normalizedSearch, overview.data?.servers]);
  const filteredExpiring = useMemo(() => (overview.data?.expiring ?? []).filter((server) => {
    return matchesIptvServerSearch(server, normalizedSearch);
  }), [normalizedSearch, overview.data?.expiring]);
  const startVoiceSearch = () => {
    ...
    recognition.onerror = (event) => { if (event.error !== "aborted") toast.error(event.error === "not-allowed" ? "Libere o microfone para usar a busca por voz." : "Não foi possível entender. Tente falar o nome ou o servidor novamente."); };
