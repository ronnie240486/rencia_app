  ...
  return (
    <AdminLayout title="Credenciais de Aplicativo">
        ...
        <section className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-foreground">Acessos do painel IPTV</h1>
                <p className="mt-1 text-sm text-muted-foreground">O login, a senha e a DNS são os dados reais XTeam da Lista Principal. O MAC é registrado no primeiro login do APK.</p>
              </div>
            </div>
                ...
                <tbody>
                  {credentials.map((credential) => (
                    <tr key={credential.id} className="border-t transition-colors hover:bg-muted/25">
                      ...
                      <td className="px-5 py-3 font-mono text-xs text-muted-foreground">{credential.dnsHost || "DNS não informado"}</td>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelectedId(null); }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Editar acesso</DialogTitle><DialogDescription>{selected ? `Usuário XTeam ${selected.username} para ${selected.nomeServer}. Para trocar a senha ou DNS, edite a Lista Principal do cliente no painel IPTV e cadastre novamente o acesso.` : ""}</DialogDescription></DialogHeader>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setSelectedId(null)}>Cancelar</Button><Button disabled={updateMutation.isPending || !selected} onClick={() => selected && updateMutation.mutate({ id: selected.id, status, active: status === "Liberado", dataExpiracao: expiration })}>{updateMutation.isPending ? "Salvando..." : "Salvar"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </AdminLayout>
  );
