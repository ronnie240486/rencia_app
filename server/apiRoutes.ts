...
import { isConfirmedListResponse, isLikelyM3uUrl, probeListUrl } from "./listHealth";
    ...
    try {
      ...
      if (currentCandidate?.url) {
        ...
        if (sameProfile.length > 1) {
          const dnsResults: Array<{ entry: typeof sameProfile[number]; result: Awaited<ReturnType<typeof probeListUrl>> }> = [];
          for (const entry of sameProfile.filter((item) => item.ativo !== false)) {
            const candidateUrl = replaceDnsHost(currentCandidate.url, entry.host);
            const result = await probeListUrl(isLikelyM3uUrl(candidateUrl) ? candidateUrl : sanitizeUrlForProbe(candidateUrl), { timeoutMs: 2500, retryOnTimeout: false, requireM3uContent: isLikelyM3uUrl(candidateUrl) });
            dnsResults.push({ entry, result });
          ...
          // Só M3U autenticada confirmada pode impedir a troca para Change Playlist.
          const workingDns = alternativeDnsResults.find(({ result }) => isConfirmedListResponse(result));
          if (workingDns) {
