import { and, count, desc, eq, gte, like, lt, or, sql, asc } from "drizzle-orm";
...
// ─── InteractivePro Config ────────────────────────────────────────────────────

export async function getInteractiveConfig(ownerId: number) {
  const db = await getDb();
  if (!db) return null;
  const { interactiveConfig } = await import("../drizzle/schema");
  return db.select().from(interactiveConfig).where(eq(interactiveConfig.ownerId, ownerId)).get();
}

export async function upsertInteractiveConfig(ownerId: number, data: {
  ...
  const existing = await db.select().from(interactiveConfig).where(eq(interactiveConfig.ownerId, ownerId)).get();
  ...
  if (existing) {
    ...
    if (data.appLogo !== undefined) updateData.appLogo = data.appLogo;
    if (data.autoplayInterval !== undefined) updateData.autoplayInterval = data.autoplayInterval;
    if (Object.keys(updateData).length > 0) {
      await db.update(interactiveConfig).set(updateData).where(eq(interactiveConfig.ownerId, ownerId));
    }
  } else {
    await db.insert(interactiveConfig).values({
      ownerId,
      backgroundUrl: data.backgroundUrl,
      appName: data.appName || "InteractivePro",
...
export async function getInteractiveBanners(ownerId: number) {
  const db = await getDb();
  if (!db) return [];
  const { interactiveBanners } = await import("../drizzle/schema");
  return db.select().from(interactiveBanners)
    .where(and(eq(interactiveBanners.ownerId, ownerId), eq(interactiveBanners.ativo, true)))
    .orderBy(asc(interactiveBanners.ordem));
}

export async function createInteractiveBanner(ownerId: number, data: {
  titulo: string;
  descricao?: string;
  tipo: "image" | "video";
  urlMedia: string;
  ...
  await db.insert(interactiveBanners).values({
    ...
    duracao: data.duracao || 5,
    ordem: data.ordem || 0,
    ativo: true,
  });
}

export async function updateInteractiveBanner(bannerId: number, ownerId: number, data: {
  titulo?: string;
  descricao?: string;
  tipo?: "image" | "video";
  urlMedia?: string;
  ...
  if (data.tipo !== undefined) updateData.tipo = data.tipo;
  if (data.urlMedia !== undefined) updateData.urlMedia = data.urlMedia;
  if (data.duracao !== undefined) updateData.duracao = data.duracao;
  if (data.ordem !== undefined) updateData.ordem = data.ordem;
  if (data.ativo !== undefined) updateData.ativo = data.ativo;
  
  if (Object.keys(updateData).length > 0) {
    await db.update(interactiveBanners)
      .set(updateData)
      .where(and(eq(interactiveBanners.id, bannerId), eq(interactiveBanners.ownerId, ownerId)));
  }
}

export async function deleteInteractiveBanner(bannerId: number, ownerId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const { interactiveBanners } = await import("../drizzle/schema");
